...
| 代码修改的强制隔离与候选测试/审查/集成 | Claude 托管流程 |
| 会话模型、保存默认、单次覆盖 | 已实现；由实际 CLI 能力决定可用模型 |
| Grok | 仅已核验的 1.0.34 只读 profile，需显式任务会话目录写授权 |
| AGY | 已核验能力的只读 profile；终端工具未建立原生验收 |
| 原生会话 resume | typed profile 未验证，保持拒绝 |
| 自动加预算/自动替换失败任务 | 不支持 |

Grok 内置工具限制为 read_file/list_dir/grep，禁用原生子任务和 web search，并传递 MCP 拒绝规则；真实 MCP 负向拒绝尚未完成原生验收。其 artifact 包含公开 assistant 文本，可能含过程说明，不能把最终回答匹配当成整个 artifact 精确匹配。
